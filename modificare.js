document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById("nume").addEventListener("click", function() {
        afiseazaOptiuni("nume", "numeNou");
    });

    document.getElementById("prenume").addEventListener("click", function() {
        afiseazaOptiuni("prenume", "prenumeNou");
    });

    document.getElementById("telefon").addEventListener("click", function() {
        afiseazaOptiuni("telefon", "telefonNou");
    });

    document.getElementById("avans").addEventListener("click", function() {
        afiseazaOptiuni("avans", "avansNou");
    });

    document.getElementById("tipEv").addEventListener("click", function() {
        afiseazaOptiuni("tipEv", "tip_eveniment");
    });

    document.getElementById("meniu").addEventListener("click", function() {
        afisareMeniu();
    });

    document.getElementById("culoare").addEventListener("click", function() {
        afiseazaOptiuni("culoare", "culoareNoua");
    });

    document.getElementById("dataEv").addEventListener("click", function() {
        afiseazaOptiuni("dataEv", "data_eveniment");
        afiseazaOptiuni("dataEv", "ora_incepere");
    });
    ...
    document.getElementById("fotograf").addEventListener("click", function() {
        afisareFotograf();
    });
});
...
function afisareMeniu() {
    // Obține elementul checkbox
    var checkbox = document.getElementById("meniu");
    var container = document.getElementById('meniuNou');
    container.innerHTML = '';
    ...
    if (checkbox.checked) {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4) {
                if (xhr.status == 200) {
                    ...
                    // Crează și adaugă dropdown-ul dinamic
                    var label = document.createElement('label');
                    label.textContent = 'Selectați un meniu nou:';
                    label.setAttribute('for', 'dinamic_select_meniu');
                    container.appendChild(label);

                    var select = document.createElement('select');
                    select.id = 'dinamic_select_meniu';
                    select.name = 'dinamic_select_meniu';
                    ...
                    // Adaugă un listener pentru a asculta schimbările în dropdown
                    select.addEventListener('change', function () {
                        var existingTable = document.getElementById('dynamic_table_meniu');
                        if (select.value !== '-') {
                            // Dacă se selectează o opțiune diferită de "-", șterge tabelul
                            if (existingTable) {
                                container.removeChild(existingTable);
        ...
        var sql = "SELECT Denumire AS Meniu, Gustare_rece AS 'Gustare rece', Peste AS Pește, Traditional AS Tradițional, Grill, Tort, Candy_bar AS 'Candy bar', Bauturi AS Băuturi, Pret AS Preț FROM Meniu WHERE IDMeniu != " + checkbox.value + " AND Tip_Eveniment = (SELECT Tip_Eveniment FROM Meniu WHERE IDMeniu = " + checkbox.value + ")";
        xhr.open("GET", "procesare_query.php?sql=" + encodeURIComponent(sql), true);
        xhr.send();
    }
}

function afisareSalon() {
    // Obține elementul checkbox
    var checkbox = document.getElementById("salon");
    var container = document.getElementById('salonNou');
    var numarLocuriInput = document.getElementById('numar_locuri').value || 0; // Obține valoarea introdusă sau folosește 0 dacă nu s-a introdus nimic
    var costPreferat = document.getElementById('cost').value || 0; // Obține valoarea introdusă sau folosește 0 dacă nu s-a introdus nimic
    container.innerHTML = '';
    ...
    if (checkbox.checked) {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4) {
                if (xhr.status == 200) {
                    ...
                    // Crează și adaugă dropdown-ul dinamic
                    var label = document.createElement('label');
                    label.textContent = 'Selectați un salon nou:';
                    label.setAttribute('for', 'dinamic_select_salon');
                    container.appendChild(label);

                    var select = document.createElement('select');
                    select.id = 'dinamic_select_salon';
                    select.name = 'dinamic_select_salon';
                    ...
                    // Adaugă un listener pentru a asculta schimbările în dropdown
                    select.addEventListener('change', function () {
                        var existingTable = document.getElementById('dynamic_table_salon');
                        if (select.value !== '-') {
                            // Dacă se selectează o opțiune diferită de "-", șterge tabelul
                            if (existingTable) {
                                container.removeChild(existingTable);
        ...
        var sql = "SELECT S.Nume_Salon AS Salon, S.Descriere, S.Nr_locuri AS 'Număr de locuri', S.Cost_inchiriere AS 'Costul închirierii' FROM Salon S WHERE S.Nr_locuri " + operatorPentruLocuri + " " + numarLocuriInput + " AND S.Cost_inchiriere " + operatorPentruCost + " " + costPreferat + " AND IDSalon != (SELECT IDSalon FROM Eveniment WHERE IDEveniment = " + checkbox.value + ") AND NOT EXISTS (SELECT 1 FROM Eveniment E WHERE S.IDSalon = E.IDSalon AND DATE(E.Data_eveniment) = (SELECT DATE(Data_eveniment) FROM Eveniment WHERE IDEveniment = " + checkbox.value + "));";
        xhr.open("GET", "procesare_query.php?sql=" + encodeURIComponent(sql), true);
        xhr.send();
    }
}

function afisareFotograf() {
    // Obține elementul checkbox
    var checkbox = document.getElementById("fotograf");
    var container = document.getElementById('fotoNou');
    var costPreferat = document.getElementById('foto').value || 0; // Obține valoarea introdusă sau folosește 0 dacă nu s-a introdus nimic
    container.innerHTML = '';
    ...
    if (checkbox.checked) {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4) {
                if (xhr.status == 200) {
                    ...
                    // Crează și adaugă dropdown-ul dinamic
                    var label = document.createElement('label');
                    label.textContent = 'Selectați o firmă nouă:';
                    label.setAttribute('for', 'dinamic_select_foto');
                    container.appendChild(label);

                    var select = document.createElement('select');
                    select.id = 'dinamic_select_foto';
                    select.name = 'dinamic_select_foto';
                    ...
                    for (var i = 0; i < options.length; i++) {
                        var option = document.createElement('option');
                        option.value = options[i].Firma;
                        option.text = options[i].Firma;
                        select.appendChild(option);
                    }
                    ...
                    // Adaugă un listener pentru a asculta schimbările în dropdown
                    select.addEventListener('change', function () {
                        var existingTable = document.getElementById('dynamic_table_foto');
                        if (select.value !== '-') {
                            // Dacă se selectează o opțiune diferită de "-", șterge tabelul
                            if (existingTable) {
                                container.removeChild(existingTable);
        ...
        var sql = "SELECT F.Nume AS Firma, F.Nr_Contact AS 'Număr de contact', F.Cost_servicii AS 'Cost servicii' FROM Fotograf F WHERE F.Cost_servicii " + operatorPentruFoto + " " + costPreferat + " AND NOT EXISTS (SELECT 1 FROM Eveniment E WHERE F.IDFotograf = E.IDFotograf AND DATE(E.Data_eveniment) = (SELECT DATE(Data_eveniment) FROM Eveniment WHERE IDEveniment = " + checkbox.value + "));";
        xhr.open("GET", "procesare_query.php?sql=" + encodeURIComponent(sql), true);
        xhr.send();
    }
}